...
from serotiny.io.dataframe import LoadColumns
...
def test_dataframe_dataset():
    ...
    loaders = {
        "A": LoadColumns(columns=["A"]),
        "B": LoadColumns(columns=["B"]),
        "AB": LoadColumns(columns=["A", "B"]),
    }
    dataframe_dataset = DataframeDataset(dataframe, loaders)

    assert dataframe_dataset[0]["A"] == data["A"][0]
    assert dataframe_dataset[0]["B"] == data["B"][0]
    assert (dataframe_dataset[0]["AB"] == dataframe[["A", "B"]].iloc[0].values).all()
