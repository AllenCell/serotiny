...
def to_onehot(labels, num_classes):
    ...
    labels = labels-1 #0 indexed (assume starting index is 1)
    labels_onehot = torch.zeros(labels.size()[0], num_classes).type_as(labels)
    labels_onehot.scatter_(1, labels.view(-1, 1), 1)

    return labels_onehot


class ConditionalImageVAE2(BaseVAE):
    def __init__(
        self,
        ...
        in_channels = in_channels + self.num_classes
        ...
        decoder = ImageDecoder(
            ...
            output_channels=orig_in_channels,
            mode=mode,
            non_linearity=non_linearity,
            skip_connections=skip_connections,
            batch_norm=batch_norm,
        ...
        self.flat_layers = [nn.Flatten(), MLP(flat_dim, latent_dim*2, hidden_layers=linear_hidden_layers)]
    ...
    def encode(self, x, **kwargs):
        ...
        onehot_condition = to_onehot(condition, self.num_classes)
        onehot_condition = onehot_condition.view(-1, self.num_classes, 1, 1, 1)
        ones = torch.ones(x.size()[0], 
                          self.num_classes,
                          x.size()[2], 
                          x.size()[3], 
                          x.size()[4],
                          dtype=x.dtype).type_as(x)

        ones = ones * onehot_condition
        x = torch.cat((x, ones), dim=1).float()

        _tmp = self.encoder(x) # # Convolutions
        _tmp = self.flat_layers[0](_tmp) # Flatten

        mu_logvar = self.flat_layers[1](_tmp) # Linear compress
    ...
    def decode(self, mu, logvar, **kwargs):
        ...
        condition = kwargs['condition']
        onehot_condition= to_onehot(condition, self.num_classes)
        z = torch.cat((z, onehot_condition), dim=1)     
        
        x_hat = self.decoder[0](z)#condition
